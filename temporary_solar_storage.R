...
generate_reduction <- function(
  ...
  if (utility_scale_storage_capacity != 0 | distributed_storage_capacity != 0) {
    ...
    if (pair_solar_with_storage) {
      ...
      solar_profile_utility <- dplyr::pull(cfs, `Utility PV`) * utility_solar_pv_capacity_mw
      solar_profile_distributed <- dplyr::pull(cfs, `Rooftop PV`) * rooftop_solar_pv_capacity_mw
      ...
      charging_tibble_8760 <- charging_tibble_8760 |>
        dplyr::mutate(
          ...
          day = lubridate::floor_date(datetime_8760, unit = "day"),
          daily_charging_both = dplyr::if_else(
            charging_indicator == "Charging",
            daily_load_reduction_both,
            0
          ),
          daily_charging_min_solar_charging = dplyr::if_else(
      ...
      charging_tibble_8760t <- charging_tibble_8760t |>
        dplyr::mutate(daily_charging_sum = abs(daily_charging_sum))


      charging_tibble_8760t <- charging_tibble_8760t |>
        dplyr::mutate(
          final_profile = dplyr::case_when(
      ...
      charging_tibble_8760 <- charging_tibble_8760 |> dplyr::mutate(solar_storage_load_reduction = dplyr::case_when(
      ...
      charging_tibble_8760 <- charging_tibble_8760 |>
        dplyr::group_by(day) |>
        dplyr::mutate(total_daily_charging = sum(solar_storage_load_reduction, na.rm = TRUE))
