...
avertr_test_annual <- function(avertr_results, avert_run_filepath) {
  ...
  error_summary_table <- test_errors |> 
  ...
  largest_absolute_errors <- test_errors |> 
    select(contains("error")) |> 
  ...
  absolute_pct_errors_above_0pt1 <- test_errors |> 
    mutate(across(contains("pct"), abs)) |> 
    filter(if_any(contains("pct"), \(x) x > 0.1))
  
  
  # If there are EGUS > 0.1%, warn and return (i.e., add to list which gets
  #   returned below)
